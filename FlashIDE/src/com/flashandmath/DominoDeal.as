...
*********************************/
	...
	btn_pullButton.x = btn_shakeButton.x;
	btn_pullButton.y = btn_shakeButton.y;
	...
	/* 
	Create a mcPlayField that we can attach all objects to. This will allow us to move the effect easily to different parts of the stage, if desired.
		*/

	var mcPlayField:Sprite = new Sprite();
	mcPlayField.x = 496;
	mcPlayField.y = 588.4;
	addChild(mcPlayField);
	...
	*/
	...
	function setupPile():void {
		...
		while (mcPlayField.numChildren>0) {
			mcPlayField.removeChildAt(0);
		}
	...
	*/
	...
	function tmDelay_Done(te:TimerEvent):void {
		...
		if (numPulled < 7) {
			...
			(in the display list) all else on the mcPlayField. */
			topDomino = arrSet.splice(arrSet.length - 1,1)[0];
			mcPlayField.setChildIndex(topDomino,mcPlayField.numChildren-1);
