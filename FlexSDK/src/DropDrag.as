package 
{
  ...
  import Table;
  public class DropDrag extends Sprite
  {
    protected const NUM_FILES:int = 10;
    protected var deleteBin:Sprite;
    
	public function DropDrag ()
	{
      dropZone = makeDropZone();
      //the delete bin should stay at the bottom
      addChildAt(dropZone, 0);
      dropZone.x = playField.width/2;
		dropZone.y = playField.height / 2;
      
      for (var i:int = 0; i < NUM_FILES; i++) {
        var file:Sprite = makeFile();
        addChild(file);
        //randomize position by looking at available stage size
        file.x = Math.random() * (playField.Width - stage.stageHeight);
        file.y = Math.random() * (playField.Height - stage.stageHeight);
